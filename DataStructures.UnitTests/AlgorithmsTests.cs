...
namespace DataStructures.UnitTests
{
    ...
    public class AlgorithmsTests
    {
        ...
        public void FindShortestPath_NonnegativeWeights_SmallGraph()
        {
            ...
            // Act
            var path1 = graph.FindShortestPath(Algorithms.GraphType.NonnegativeWeights, node1, node3, out double pathWeight1);
            var path2 = graph.FindShortestPath(Algorithms.GraphType.NonnegativeWeights, node2, node1, out double pathWeight2);
            var path3 = graph.FindShortestPath(Algorithms.GraphType.NonnegativeWeights, node3, node3, out double pathWeight3);
            ...
            Assert.AreEqual(12, pathWeight1);
            Assert.AreEqual(double.PositiveInfinity, pathWeight2);
            Assert.AreEqual(0, pathWeight3);
        }
        ...
        public void FindShortestPath_NonnegativeWeights_LargeGraph()
        {
            ...
            // Act
            var path1 = graph.FindShortestPath(Algorithms.GraphType.NonnegativeWeights, node2, node9, out double pathWeight1);

            // Assert
            var expectedPath1 = new List<IReadOnlyEdge<int, int>> { edge3, edge6, edge10, edge16 };
            CollectionAssert.AreEqual(expectedPath1, path1.ToList());
            Assert.AreEqual(25, pathWeight1);
        }
    }
}